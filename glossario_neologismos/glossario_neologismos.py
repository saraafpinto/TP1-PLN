...
for c in conceitos[1:]:
    ...
    if len(elems) > 1:
        # 1. TRATAMENTO DA DESIGNAÇÃO E GÉNERO
        ...
        if len(corpo_split) >= 2:
            ...
            # 2. LIMPEZA DAS TRADUÇÕES (Retirar [ing] e [esp])
            ...
            # 3. EXTRAÇÃO DE SIGLA, DESCRIÇÃO E INF. ENCICL.
            # Procurar por "Sigla: XXX"
            pesquisa = ""
            match_pesquisa = re.search(r'[“"](.*?)[”"]', resto_corpo, flags=re.DOTALL)
            if match_pesquisa:
                pesquisa = match_pesquisa.group(1).replace('\n', ' ').strip()
                # Opcional: remover a citação do corpo de texto para a definicao ficar limpa
                resto_corpo = resto_corpo.replace(match_pesquisa.group(0), "").strip()
            ...
            if match_sigla:
                ...
                # Remove a linha da sigla para não sujar a definicao
